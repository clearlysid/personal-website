...
const StyledSmoothScroll = styled.div`
    position: relative;
    min-height: 100vh;
    padding: 120px 0;
`
...
export default function SmoothScroll({ children }) {
    ...
    useEffect(() => {
        ...
        const smoothScrolling = () => {
            ...
            scrollConfig.previous +=
                (scrollConfig.current - scrollConfig.previous) *
                scrollConfig.ease
            scrollConfig.rounded = Math.round(scrollConfig.previous * 100) / 100
            const skew =
                Number(
                    (scrollConfig.current - scrollConfig.rounded) /
                        window.innerWidth
                ) * scrollConfig.skew
            if (scrollWrapper) {
                scrollWrapper.style.transform = `translate3d(0, -${scrollConfig.rounded}px, 0) skewY(${skew}deg)`
            }
    ...
    return (
        <StyledSmoothScroll className="smooth-scroll" ref={scroller}>
            {children}
        </StyledSmoothScroll>
    )
}
