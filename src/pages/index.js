...
export default function IndexPage({ data }) {
    ...
    return (
        <App>
             ...
             <motion.header className="header" key="header" exit={{opacity: 0}} transition={transition}>
                 {/* 
                ...
                <h2 className="site-header-text">
                    <span>Siddharth</span> builds prototypes as a <span>UX Engineer</span> Intern at <span>Headout.</span> Here’s what he’s been writing about lately...</h2>
                    {/* Siddharth interns as a <span>UX Engineer</span> and builds prototypes at <span>Headout</span>. Here’s what he’s been writing about lately...</h2> */}
            </motion.header>
                ...
                {data.allNotionPageBlog.edges.map(edge => (
                    <SwiperSlide key={edge.node.slug}>
                        <Link className="article-card" to={`/blog/${edge.node.title.toLowerCase().replace(/[^\w ]+/g, '').replace(/ +/g, '-')}`}>
                            <motion.div key={edge.node.title} className="article-image" whileHover={{ scale: 0.98 }} transition={transition}>
