            ...
            case "page":
                if (level === 0) {
                    if (fullPage) {
                        ...
                        const { page_icon, page_cover, page_cover_position, page_full_width, page_small_text } = blockValue.format || {};
                        const coverPosition = (1 - (page_cover_position || 0.5)) * 100;

                        return (
                            <div className="notion">
                                {!hideHeader && <PageHeader blockMap={blockMap} mapPageUrl={mapPageUrl} mapImageUrl={mapImageUrl} />}
                                {page_cover && <img src={mapImageUrl(page_cover, block)} alt={getTextContent(blockValue.properties.title)}
                                        className="notion-page-cover" style={{ objectPosition: `center ${coverPosition}%` }} />}
                                <main className={classNames(
                                        "notion-page", !page_cover && "notion-page-offset", page_full_width && "notion-full-width", page_small_text && "notion-small-text"
                                    )}>
                                    {page_icon && <PageIcon className={ page_cover ? "notion-page-icon-offset" : undefined }
                                            block={block} big mapImageUrl={mapImageUrl} />}
